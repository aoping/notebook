...
// 版本控制
var resourceVersion = '12312443243205';
                ...
                if (this.needUpdate === true) {
                    ...
                    return (function() {
                        ...
                        for (var i = 0; i < _self.resourceJavascriptList.length; i++) {
                            ...
                            // 去读取本地文件
                            var type = _self.resourceJavascriptList[i]['type'];
                            switch (type) {
                                case 'javascript':
                                    window.mDomUtils.addJavascriptByInline(scriptId);
                                    break;
                                case 'css':
                                    window.mDomUtils.addCssByInLine(scriptId);
                                    break;
                            }
                        }
                    })();
                        ...
                        for (var i = 0; i < resourceJavascriptList.length; i++) {
                            ...
                            switch (type) {
                                case 'javascript':
                                    ...
                                    break;
                                case 'css':
                                    window.mDomUtils.addCssByLink(scriptId, resourceJavascriptList[i]['url']);
                                    break;
                            }
...
window.mDomUtils = {
    ...
    addCssByInLine: function(scriptId) {
        var doc = document;
        var link = doc.createElement('link');
        var cssString = localStorage.getItem(scriptId);
        link.setAttribute('type', 'text/css');
        link.setAttribute('rel', 'stylesheet');
